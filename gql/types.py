...
class TopicType(DjangoObjectType, SEOType):
    ...
    def resolve_references(self, info, *_):
        #print(self.content_html)
...
class DirectorPersonMixType(DjangoObjectType, SocialMediaType, SEOType, StatisticsType):
    ...
    def resolve_preview_movies(self, info, *_):
        ...
        highest_movies = cqs.order_by("-movie__imdb_rating").values_list("movie__id", flat=True)[:5]
        movie_ids = list(set(list(last_movie) + list(highest_movies)))[:4]
...
class ProfileType(DjangoObjectType, SocialMediaType, SEOType):
    ...
    def resolve_mail_subs_status(self, info, *_):
...
class CustomMovieType(graphene.ObjectType, SocialMediaType, SEOType):
    ...
    def resolve_topics(self, info):
        ...
        or_topics =  list(map(lambda x: x.topic, self.movie.in_topics.all().exclude(topic__cover_poster=""))) 
        return list(set(un_topics + or_topics))
    ...
    def resolve_release(self, info):
        if self.movie.release:
            return self.movie.release

    def resolve_genres(self, info):
        if len(self.movie.genre_names) > 0:
            return self.movie.genre_names
        return []
    ...
    def resolve_seo_short_description(self, info, *_):
        ...
        if tag_qs.exists():
            ...
            dsc = dsc + f"{self.movie.name} "
            if self.movie.videos.all().count()> 0:
                dsc = dsc + f" trailer, "
