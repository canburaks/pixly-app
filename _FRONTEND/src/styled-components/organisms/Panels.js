...
const ListName = ({ item }) => <Text color="lightDark1" textShadow="dark" fontSize={[15, 15, 18, 20, 22]} fontWeight="500">{item.relatedPersons.length > 0 ? <><DirectorLink mr={[0]} director={item.relatedPersons[0]} />{item.name.split(item.relatedPersons[0].name)[1]}</> : item.name}</Text>
