...
export const SignupForm = (props) => {
    ...
    return (
        <form onSubmit={handleSubmit(props.onSubmit)} className="frm-form" >
            <input className="frm-item" 
                ref={register({required: true, maxLength: 40})} 
                type="text" 
                placeholder="Full Name" name="Full Name" 
                />
            <input className="frm-item" 
                ref={register({required: true, maxLength: 16, pattern: /^[a-zA-Z0-9]+([a-zA-Z0-9](_|-| )[a-zA-Z0-9])*[a-zA-Z0-9]+$/i})} 
                type="text" 
                placeholder="Username" name="Username" 
                />
            <input className="frm-item" 
                ref={register({required: true, minLength: 4, maxLength: 40, pattern: /^(("[\w-\s]+")|([\w-]+(?:\.[\w-]+)*)|("[\w-\s]+")([\w-]+(?:\.[\w-]+)*))(@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i})} 
                type="email" 
                placeholder="Email" name="Email" 
                />
            <input className="frm-item" 
                ref={register({required: true, minLength: 8, maxLength: 18, pattern: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,18}$/i})} 
                type="text" 
            ...
            <input className="frm-item" 
                ref={register({required: true, minLength: 8, maxLength: 18, pattern: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,18}$/i})} 
                type="text" 
                placeholder="Re-Password" name="Re-Password" 
                />
...
export const ProfileInfoForm = (props) => {
    ...
    return(
        <Mutation mutation={PROFILE_MUTATION} onCompleted={data => (console.log("completed"), state.methods.toggleModal())}>
            {(mutation) =>(
                <ProfileForm {...props} onSubmit={qv => mutation({variables:{...qv}})} />
            )}
        </Mutation>
    )
}
...
const ProfileForm = (props) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="frm-form profile-info-form" >
            ...
            <div className="frm-box">
                ...
                <Input className="frm-item" 
                    ...
                    name="Full Name" ref={register({maxLength: 40})} 
                />
            </div>

            <div className="frm-box">
                <label className="frm-label">Bio</label>
                <Input className="frm-item"  name="Bio" type="text" 
                    ref={register({maxLength: 140})} defaultValue={props.profile.bio} />
            </div>

            <div className="frm-box">
                <label className="frm-label">Country</label>
                <select className="frm-select frm-item" name="Country" ref={register} defaultValue={props.profile.country && props.profile.country[1]}>
                    {Object.keys(countries).map(c => <option value={c} key={c} >{countries[c]}</option>)}
                </select>
            </div>
